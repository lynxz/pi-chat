// Phase-5 wiring test: verifies the auto-reply path end-to-end.
//
...
// `sendUserMessage` fires exactly once for a match (Phase 5's "only one
// turn at a time" promise). Also exercises:
//
//   - `shouldAutoReply` decisioning (filters.ts)
//   - `AutoReplyWorker` enqueue + pump (§6.4 / §11)
//   - the `agent_end` reset hook
//   - the inbound self-echo / dedupe / cooldown filters (§6.4)
...
function buildMount(
  ...
  // still propagating.
  const room = overrides.roomName ?? `phase5-${Math.random().toString(36).slice(2, 8)}`;
...
describe("Phase 5 — auto-reply inbound wiring", () => {
  it("matching @-mention triggers sendUserMessage exactly once (serial queue)", async () => {
    ...
    // happen yet — that's the §11 protection. We use a different sender
    // (`carol`) so the per-sender cooldown filter doesn't drop the
